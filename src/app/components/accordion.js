  ...
  return (
    <div>
      {content.map((card, index) => (
        <div key={index} className="py-4 border-b-2 border-gray">
          <button onClick={() => toggleAccordion(index)} className="flex items-center justify-between w-full">
            <span key={index} className="text-lg text-gray">{card.question}</span>
            <svg className="fill-razzmataz shrink-0 ml-8" width="16" height="16" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className={` grid overflow-hidden transition-all duration-300 ease-in-out text-slate-500 ${openAccordion === index ? "my-2 grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
